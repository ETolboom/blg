...
class Bpmn:
            ...
            for element in pool_elements:
                ...
                for child in process_element:
                    ...
                    if child.get("attachedToRef") == attached_to_ref:
                        ...
                        if (
                            boundary_event_id
                            and boundary_event_id not in element.boundary_events
                        ):
    ...
    def find_task(
        self, task_label: str, match_threshold: float = 0.6
    ) -> tuple[PoolElement, float] | None:
    ...
    def _search_task_in_path(
        ...
        if not target_element:
            logger.debug(
                "[_search_task_in_path] Target element '%s' not found",
                target_element_id,
            )
            return -1, None, 0.0

        logger.debug(
            "[_search_task_in_path] Distance %d: Found element '%s' (type: %s, event_def: %s)",
            current_distance,
            target_element.label,
            target_element.name,
            target_element.event_definition,
        )
        ...
        if is_boundary_event:
            ...
            if "boundary" in task_label_lower and "event" in task_label_lower:
                ...
                target_element.event_definition
                and target_element.event_definition in task_label_lower
            ):
                is_match = True
                logger.debug(
                    "[_search_task_in_path] Matched boundary event type '%s'",
                    target_element.event_definition,
                )
            ...
            similarity_matrix = create_similarity_matrix(
                [task_label], [target_element.label]
            )
            similarity_score = similarity_matrix[0, 0].item()
            logger.debug(
                "[_search_task_in_path] Comparing '%s' with '%s': %.3f",
                target_element.label,
                task_label,
                similarity_score,
            )
            ...
            if similarity_score >= match_threshold:
                logger.debug(
                    "[_search_task_in_path] MATCH! Score %.3f >= threshold %.3f",
                    similarity_score,
                    match_threshold,
                )
        ...
        if current_distance >= max_distance:
            logger.debug(
                "[_search_task_in_path] Reached max_distance (%d)", max_distance
            )
            return -1, None, 0.0
        ...
        if len(target_element.outgoing) == 1:
            return self._search_task_in_path(
                target_element.outgoing[0],
                pool,
                task_label,
                match_threshold,
                max_distance,
            ...
            logger.debug(
                "[_search_task_in_path] Element is a gateway with %d outgoing edges, cannot continue (ambiguous path)",
    ...
    def find_next_task(
        ...
        logger.debug(
            "[find_next_task] Starting search from element ID: %s", starting_element_id
        )
        logger.debug(
            "[find_next_task] Looking for TASK: '%s' (max_distance=%d, threshold=%.3f)",
            task_label,
            max_distance,
            match_threshold,
        )
        ...
        if not lookup:
            raise ValueError(
                f"Starting element with id '{starting_element_id}' not found"
            )
        pool_for_element, starting_element = lookup

        logger.debug(
            "[find_next_task] Starting element: '%s' (ID: %s)",
            starting_element.label,
            starting_element.id,
        )
        logger.debug(
            "[find_next_task] Outgoing connections: %s", starting_element.outgoing
        )
        logger.debug(
            "[find_next_task] Boundary events: %s", starting_element.boundary_events
        )
        ...
        while visit_count < max_distance:
            ...
            if is_first_iteration and current_element.boundary_events:
                logger.debug(
                    "[find_next_task] Element has %d boundary event(s), checking them first",
                ...
                for boundary_event_id in current_element.boundary_events:
                    ...
                    if not boundary_event:
                        logger.debug(
                            "[find_next_task] Boundary event '%s' not found, skipping",
                            boundary_event_id,
                        )
                        continue

                    logger.debug(
                        "[find_next_task] Checking boundary event with type '%s' (ID: %s)",
                        boundary_event.event_definition,
                        boundary_event_id,
                    )
                    ...
                    # Match specific event types (message, timer, error, signal, etc.)
                        ...
                        if boundary_event.event_definition in task_label_lower:
                            is_boundary_event_match = True
                            logger.debug(
                                "[find_next_task] Matched event type '%s' in '%s'",
                                boundary_event.event_definition,
                                task_label,
                            )
                    ...
                    if boundary_event.outgoing:
                        for outgoing_flow_id in boundary_event.outgoing:
                            logger.debug(
                                "[find_next_task] Trying boundary event outgoing flow ID: %s",
                                outgoing_flow_id,
                            )
                            result = self._search_task_in_path(
                                outgoing_flow_id,
                                pool_for_element,
                                task_label,
                                match_threshold,
                ...
                logger.debug(
                    "[find_next_task] No match found in boundary events, checking normal outgoing flows"
                )
            ...
            if not current_element.outgoing:
                ...
                logger.debug(
                    "[find_next_task] No outgoing edges from '%s', stopping",
            ...
            if not is_first_iteration and len(current_element.outgoing) != 1:
                ...
                logger.debug(
                    "[find_next_task] Intermediate element has %d outgoing edges (expected 1), stopping",
            ...
            if is_first_iteration and len(current_element.outgoing) > 1:
                logger.debug(
                    "[find_next_task] Starting element has %d outgoing edges, checking all paths",
                    len(current_element.outgoing),
                )
                for outgoing_flow_id in current_element.outgoing:
                    logger.debug(
                        "[find_next_task] Trying flow ID: %s", outgoing_flow_id
                    )
            ...
            if not flow:
                logger.debug(
                    "[find_next_task] Flow '%s' not found in pool flows, stopping",
                    outgoing_flow_id,
                )
                return -1, None, 0.0
            ...
            if not next_element:
                logger.debug(
                    "[find_next_task] Target element '%s' not found, stopping",
                    flow.target,
                )
                return -1, None, 0.0
            ...
            visit_count += 1
            logger.debug(
                "[find_next_task] Visit %d: Found element '%s' (type: %s)",
                visit_count,
                next_element.label,
                next_element.name,
            )
            ...
            if next_element.label:
                similarity_matrix = create_similarity_matrix(
                    [task_label], [next_element.label]
                )
                similarity_score = similarity_matrix[0, 0].item()

                logger.debug(
                    "[find_next_task] Comparing '%s' with '%s': %.3f",
                    next_element.label,
                    task_label,
                    similarity_score,
                )
                ...
                if similarity_score >= match_threshold:
                    logger.debug(
                        "[find_next_task] MATCH! Score %.3f >= threshold %.3f",
                        similarity_score,
                        match_threshold,
                    )
                    ...
                    logger.debug(
                        "[find_next_task] No match (score %.3f < threshold %.3f)",
                        similarity_score,
                        match_threshold,
                    )
            else:
                logger.debug(
                    "[find_next_task] Element has no label, skipping comparison"
                )

            # 5. If visit count and max_distance are equal, stop
            if visit_count >= max_distance:
                logger.debug(
                    "[find_next_task] Reached max_distance (%d), stopping", max_distance
                )
                return -1, None, 0.0
    ...
    def _check_gateway_match(
        self,
        gateway: PoolElement,
        normalized_gateway_type: str,
        expected_outcomes: int,
        pool: Pool,
        gateway_label: str = "",
        outcome_labels: list[str] | None = None,
        check_gateway_label: bool = False,
        check_outcome_labels: bool = False,
        match_threshold: float = 0.8,
    ) -> bool:
        ...
        if not gateway_type_match:
            logger.debug(
                "[_check_gateway_match] Gateway type mismatch: expected '%s', got '%s'",
        ...
        if not outcomes_match:
            logger.debug(
                "[_check_gateway_match] Outcomes mismatch: expected %d, got %d",
                expected_outcomes,
                num_outgoing,
            )
            return False
        ...
        if check_gateway_label:
            if not gateway.label:
                logger.debug(
                    "[_check_gateway_match] Gateway has no label, but label check is required"
                )
                return False
            ...
            logger.debug(
                "[_check_gateway_match] Gateway label similarity: '%s' vs '%s' = %.3f",
                gateway.label,
                gateway_label,
                label_score,
            )
            ...
            if label_score < match_threshold:
                logger.debug(
                    "[_check_gateway_match] Gateway label score %.3f < threshold %.3f",
                    label_score,
                    match_threshold,
                )
                return False
        ...
        if check_outcome_labels and outcome_labels:
            ...
            logger.debug(
                "[_check_gateway_match] Gateway outgoing flow labels: %s", flow_labels
            )
            logger.debug(
                "[_check_gateway_match] Expected outcome labels: %s", outcome_labels
            )

            # Check if we have the right number of flow labels
            if len(flow_labels) != len(outcome_labels):
                logger.debug(
                    "[_check_gateway_match] Number of flow labels (%d) != expected (%d)",
                    len(flow_labels),
                    len(outcome_labels),
                )
                return False
            ...
            for i, expected_label in enumerate(outcome_labels):
                ...
                if not expected_label or expected_label.strip() == "":
                    logger.debug(
                        "[_check_gateway_match] Outcome %d has empty label, accepting without matching",
                        i,
                    )
                ...
                for j, flow_label in enumerate(flow_labels):
                    ...
                    similarity_matrix = create_similarity_matrix(
                        [expected_label], [flow_label]
                    )
                    score = similarity_matrix[0, 0].item()
                    logger.debug(
                        "[_check_gateway_match] Comparing outcome '%s' with flow '%s': %.3f",
                        expected_label,
                        flow_label,
                        score,
                    )
                ...
                if not found_match:
                    logger.debug(
                        "[_check_gateway_match] Could not find match for expected outcome '%s'",
                        expected_label,
                    )
                    return False
    ...
    def find_next_gateway(
        ...
        logger.debug(
            "[find_next_gateway] Starting search from element ID: %s",
            starting_element_id,
        )
        logger.debug(
            "[find_next_gateway] Looking for GATEWAY: type=%s, expected_outcomes=%d (max_distance=%d)",
            gateway_type,
            expected_outcomes,
            max_distance,
        )
        logger.debug(
            "[find_next_gateway] Check gateway label: %s ('%s')",
            check_gateway_label,
            gateway_label,
        )
        logger.debug(
            "[find_next_gateway] Check outcome labels: %s (%s)",
        ...
        normalized_gateway_type = gateway_type_mapping.get(
            gateway_type.lower(), gateway_type.lower()
        )
        logger.debug(
            "[find_next_gateway] Normalized gateway type: '%s'", normalized_gateway_type
        )
        ...
        logger.debug(
            "[find_next_gateway] Starting element: '%s' (ID: %s)",
            starting_element.label,
            starting_element.id,
        )
        logger.debug(
            "[find_next_gateway] Outgoing connections: %s", starting_element.outgoing
        )
        logger.debug(
