...
async def get_active_project(request: Request) -> dict:
    """Return the currently active project (or null if none selected)."""
    return {"active_project": request.app.state.active_project}


@router.post("/projects/{name}/select")
async def select_project(name: str, request: Request) -> dict:
    """Select an existing project, (re)initializing app state for it."""
    try:
        set_active_project(request.app, name)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"Project '{name}' not found")
    return {"message": f"Project '{name}' selected", "active_project": name}
...
async def create_project(body: CreateProjectRequest, request: Request) -> dict:
    ...
    set_active_project(request.app, name)
    return {"message": f"Project '{name}' created", "active_project": name}
