...
export default function CounterExampleVisualizer(
  ...
  // BLG: the replay is triggered programmatically (via the START event fired by
  // useCounterExampleReplay), not by the upstream `analysis.done` DOM wiring. We
  // keep the last property so the Restart control can replay it.
  let lastProperty;
